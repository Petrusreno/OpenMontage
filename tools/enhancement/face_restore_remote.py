"""Remote GPU face restoration — delegates to gpu-worker via gpu-bridge.

Drop-in companion to face_restore.py (provider="codeformer").
...
class FaceRestoreRemote(BaseTool):
    name = "face_restore_remote"
    version = "0.1.0"
    ...
    # Matches face_restore.py input_schema exactly
    input_schema = {
        "type": "object",
        "required": ["input_path"],
        "properties": {
            ...
            "fidelity": {
                "type": "number",
                "default": 0.5,
                "description": "0 = max quality, 1 = max fidelity (CodeFormer only)",
            },
            "upscale": {
                "type": "integer",
                "default": 2,
    ...
    side_effects = ["writes restored image to output_path", "starts GCP VM if stopped"]
    best_for = ["face restoration on CUDA GPU when local GPU unavailable"]
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=False, error=f"Remote face-restore failed: {exc}")
